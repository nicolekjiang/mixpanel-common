...
export function downloadData(endpoint, params, method='POST') {
  ...
  postForm.innerHTML = Object.keys(params).map(param => {
    switch (param) {
      case 'script':
        return `<textarea name="script">${htmlEncodeString(params[param])}</textarea>`;
      case 'params':
        return `<input type="hidden" name="params" value="${htmlEncodeString(params[param])}"/>`;
      default:
        return `<input type="hidden" name="${param}" value="${params[param]}"/>`;
    }
  }).join('');

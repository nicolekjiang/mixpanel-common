 ...
 * @param {string} utcOffset - the starting date of the range - can be Date or timestamp
...
export function localizedDate(utcOffset, date) {
...
export function relativeToAbsoluteDate(relativeDateInt, unit, {utcOffset=null}={}) {
  let date = null;
  const now = Number.isInteger(utcOffset) ? moment(localizedDate(utcOffset)) : moment();
...
export function normalizeDateStrings(dates=[], {utcOffset=null}={}) {
  const now = Number.isInteger(utcOffset) ? localizedDate(utcOffset) : new Date();
  return dates
    .map(parseDate)
    .filter(Boolean)

...
import { getIconForEvent, getIconForPropertyType, renameEvent, renameProperty } from '../../report/util';
...
export default class ItemsMenu extends Component {
  ...
  getProcessedSections() {
    return this.state.sections.map(section => {
      const items = section.items.map(item => {
        if (item.itemType === this.ITEM_TYPE_EVENT) {
          return this.wrapEvent(item);
        } else if (item.itemType === this.ITEM_TYPE_PROPERTY) {
          return this.wrapProperty(item);
        } else {
  ...
  wrapEvent(event) {
    return extend({
      label: event.label || renameEvent(event.name),
      icon: event.icon || getIconForEvent(event),
      wrapped: event,
    }, event);
  }
  ...
  wrapProperty(property) {
    return extend({
      label: property.label || renameProperty(property.name),
      icon: property.icon || getIconForPropertyType(property.type),
      wrapped: property,
    }, property);
  }
  ...
  getFilteredSections() {
    ...
    return processedSections.map(section => {
      ...
      extend(section, {items: section.items.map(item =>
  ...
  /**
   * filter item list against this.state.filter, and add match

...
import {truncateMiddle} from  '../../util/index.js';
import {stringFilterMatches} from  '../../util/string.js';
import template from './index.jade';
import css from './index.styl';

export default registerMPElement(`mp-bookmarks-widget`, class extends Component {
  get config() {
    return {
      css,
      template,
      ...
      helpers: {
        ...
        handleDropMenuChange: e => {
          ...
          if (e.detail.state === `open`) {
            this.el.querySelector(`.mp-bm-menu-search`).focus();
  ...
  _filterBookmarks() {
    const filteredBookmarks = this.state.bookmarks
      .filter(b => stringFilterMatches(b.name, this.state.filterText))
  ...
  _setActiveBookmark(activeBookmark, scrollIntoView=true) {
    ...
    if (scrollIntoView) {
      window.requestAnimationFrame(() => {
        const activeEl = this.el.querySelector(`mp-list-item[active="true"]`);
        if (activeEl) {

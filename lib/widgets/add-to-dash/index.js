...
export default registerMPElement(`add-to-dash`, class extends Component {
    ...
    if (tracking.length) {
      tracking.forEach(trackingObj => {
        ...
        if (alias.length) {
          ...
          mixpanel.track(`${eventPrefix} ${eventName}`, properties);
        }
      });
    } else {
      mixpanel.track(`[Web Dash] ${eventName}`, properties);
    }
  }
  ...
  attachedCallback() {
    ...
    if (this.getAttribute(`tracking`)) {
      ...
      tracking.forEach(trackingObj => {
        if (!mixpanel[trackingObj.alias]) {
          initMixpanel(trackingObj.token, trackingObj.alias);
        }
      });

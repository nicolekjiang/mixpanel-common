...
export default registerMPElement(`mp-tooltip`, class extends Component {
  ...
  show() {
    ...
    this.showQueue = this.showQueue.then(() => this.update({
      visibility: VISIBILITY_TRANSITIONING,
      placement,
    }));

    this.showQueue = this.showQueue.then(() => new Promise(resolve => window.requestAnimationFrame(() => {
  ...
  hide() {
    this.showQueue = this.showQueue.then(() => this.update({visibility: VISIBILITY_HIDDEN}));
  }

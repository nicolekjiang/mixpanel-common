...
import template from './alert-modal.jade';

import css from './modal.styl';

document.registerElement('mp-alert-modal', class extends Component {
  get config() {
    return {
      css,
      template,
  ...
  attachedCallback() {
    ...
    if (this.isAttributeEnabled('relative')) {
      this.el.querySelector('.mp-alert-modal').classList.add('absolute');
    }
